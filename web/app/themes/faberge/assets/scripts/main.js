...
var mySwiper = new Swiper('#slider-variation', {
    direction:'vertical',
    slidesPerView:'auto',
    autoHeight:'auto',
...
/*
var zoomImage=$("a[data-rel^='prettyPhoto'],a.zoom").attr('href');
$(  '.variations_form' ).on( 'woocommerce_variation_has_changed', function( event ) {
 if (document.querySelector("a[data-rel^='prettyPhoto'],a.zoom").href.indexOf('zoom')===-1)document.querySelector("a[data-rel^='prettyPhoto'],a.zoom").href=zoomImage;
})
*/
...
$(  '.variations_form' ).on( 'woocommerce_variation_select_change',function(){
  ...
  $.fn.wc_variations_image_update = function( variation ) {
    var $form             = this,
      ...
      variation_link     = variation_link[variation_link.length-4]==='.'?variation_link.substr(0,variation_link.length-4)+'_zoom'+variation_link.substr(variation_link.length-4):variation_link+'_zoom';
    ...
    $('.variations-thumbs').children().filter(function() {
        ...
        function variationHandler(e){
          ...
          bigImage.fadeOut(function(){
            ...
            variations.children().removeClass('active');
        ...
        $('.variation').click(variationHandler);
        ...
        function popstateCallback(event) {
          if (!!event.state) {
              ajaxPush(event.state.href,false, '');
          } else if (safariBug) {
          ...
          $.ajax({
            ...
            success:function(returned) {
                ...
                $( '#content' ).empty().html(returned);
                  ajaxPush(e.currentTarget.href,true,'')
                    $('.related.products .type-product>a').click(ajaxCallback);
                    $('.variation').click(variationHandler);
                $form = $(  '.variations_form' );
                if ($form) {
...
var $j = jQuery.noConflict(), _popin, _brandNav, _navigation, _articlesList, _carrousel_array, _scroll = 0, _easingScroll = 0, _tempScroll = -1, _shoppingbagpreview;
